...
defined('BASEPATH') OR exit('No direct script access allowed');
    ...
    class Brands extends CI_Controller 
    {
        ...
        public function add() 
        {
            ...
            $output['size_type_of_half'] = false;
            $output['size_in_ml_of_half'] = '';
            $output['price_of_half'] = '';
            $output['size_type_of_quarter'] = false;
            $output['size_in_ml_of_quarter'] = '';
            ...
            if ($_POST) {
                ...
                if (isset($_POST['size_type_of_full'])) 
                {
                  ...
                  $this->form_validation->set_rules('price_of_full', 'Full Price', 'trim|required|numeric');
                }
                if(isset($_POST['size_type_of_half'])) 
                {
                  $this->form_validation->set_rules('size_in_ml_of_half', 'Half Quantity', 'trim|required|numeric');
                  $this->form_validation->set_rules('price_of_half', 'Half Price', 'trim|required|numeric');
                }
                if (isset($_POST['size_type_of_quarter'])) 
                {
                  ...
                  $this->form_validation->set_rules('price_of_quarter', 'Quarter Price', 'trim|required|numeric');                  
                }
                ...
                if($this->form_validation->run())
                {
                    ...
                    if($this->input->post('size_type_of_full'))
                    {
                      ...
                      $dbdata['price'] = $this->input->post('price_of_full');
                    ...
                    {   
                      ...
                      $dbdata['price'] = $this->input->post('price_of_half');
                      $dbdata['brand_id'] = $brand_id;
                      $this->brands->add_brand_category($dbdata);
                    }
                    if (isset($_POST['size_type_of_quarter'])) 
                    {
                      ...
                      $dbdata['price'] = $this->input->post('price_of_quarter');
                      $dbdata['brand_id'] = $brand_id;              
                      $this->brands->add_brand_category($dbdata);
                    }
        ...
        public function update($brand_id)
        {
          ...
          $output['page_title'] = "Brands";
            ...
            if($_POST)
            {   
                $output['brand_name'] = $this->input->post('brand_name');
                $output['size_type_of_full'] = $this->input->post('size_type_of_full');
                $output['size_in_ml_of_full'] = $this->input->post('size_in_ml_of_full');
                $output['price_of_full'] = $this->input->post('price_of_full');
                $output['size_type_of_half'] = $this->input->post('size_type_of_half');
                $output['size_in_ml_of_half'] = $this->input->post('size_in_ml_of_half');
                $output['price_of_half'] = $this->input->post('price_of_half');
                $output['size_type_of_quarter'] = $this->input->post('size_type_of_quarter');
                $output['size_in_ml_of_quarter'] = $this->input->post('size_in_ml_of_quarter');
                $output['price_of_quarter'] = $this->input->post('price_of_quarter');
                $output['left_menu'] = "";
                $output['left_submenu'] = "";
                $output['page_title'] = "";
